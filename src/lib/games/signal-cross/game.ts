...
type AgencyQuestion = {
  text: string;
  choices: { label: string; tag: unknown }[];
  correctIdx: number;
};
type Connection = {
  byPlayer: string;
...
type NetObj = {
  ...
  clientConns: Map<string, DataConnection>;
...
type GameState = {
  ...
  _timeoutCount?: number;
  _agencyNextAt?: number;
};
...
function makeSlip(call: CallDef, levelIdx: number) {
  ...
  if (forgedName) {
    ...
    if (pool.length)
      callerName =
        CHARS[pool[Math.floor(Math.random() * pool.length)] ?? call.from]?.name ?? call.from;
  }
  return {
...
/* ---------- Utilities ---------- */
function myPlayer() {
  return state.players.find((p) => p.id === Net.myId);
}
...
function hostHandleAction(playerId: string | null, msg: Record<string, unknown>) {
  ...
  if (msg.type === "select") {
    ...
    const ticket = state.tickets.find(
      (t) => t.from === msg.plugId && t.status === "ringing" && t.kind !== "agency"
    );
    if (!ticket) return;
    ...
    const ticket = state.tickets.find(
      (t) => t.id === msg.ticketId && t.status === "ringing" && t.kind === "agency"
    );
    if (!ticket) return;
...
function applyStateLocally(snap: Partial<GameState> & Record<string, unknown>) {
  Object.assign(state, {
    ...
    teamChaos: snap.teamChaos,
    teamPenalty: snap.teamPenalty,
    correctCount: snap.correctCount,
    tickets: snap.tickets,
    players: snap.players,
    log: snap.log,
    levelTitle: snap.levelTitle,
    levelSubtitle: snap.levelSubtitle,
...
Net.onMessage = (peerId, rawMsg) => {
  ...
  if (Net.isHost) {
    ...
    if (msg.type === "state")
      applyStateLocally(msg as Partial<GameState> & Record<string, unknown>);
...
function onPlugClick(plugId: string) {
  ...
  if (state.gameMode === "supervisor") {
    if (ticket.approval === "awaiting-stamp") {
      if (me.id === state.supervisorId) {
        openSlipModal(ticket, "supervisor");
        sfx.select();
      } else {
        sfx.denied();
  ...
  if (state.gameMode === "verify" && ticket.approval === "pending") {
    if (ticket.reviewer && ticket.reviewer !== me.id) {
      sfx.denied();
...
function bindUI() {
  on("host-btn", "click", () => {
    ...
    Net.host(name, () => {
      ...
      const rc = $("room-code");
      if (rc) rc.textContent = Net.roomCode;
      const hr = $("hud-room");
      if (hr) hr.textContent = Net.roomCode;
      showScreen("lobby");
      renderLobby();
    });
  });
  ...
  on("join-btn", "click", () => {
    ...
    if (!code) {
      Net.status("enter a room code first", "err");
      return;
    }
    Net.status("connecting...", "");
    Net.join(code, name, () => {
      const rc2 = $("room-code");
      if (rc2) rc2.textContent = code;
      const hr2 = $("hud-room");
      if (hr2) hr2.textContent = code;
      showScreen("lobby");
      renderLobby();
    });
  });
  ...
  on("slip-approve", "click", () => {
    ...
    if (state.gameMode === "verify")
      sendAction({ type: "verifyDecision", ticketId: tid, decision: "approve" });
    else if (state.gameMode === "supervisor")
      sendAction({ type: "stamp", ticketId: tid, decision: "approve" });
    sfx.stamp();
    closeSlipModal();
  });
  on("slip-deny", "click", () => {
    ...
    if (!tid) return;
    if (state.gameMode === "verify")
      sendAction({ type: "verifyDecision", ticketId: tid, decision: "deny" });
    else if (state.gameMode === "supervisor")
      sendAction({ type: "stamp", ticketId: tid, decision: "deny" });
    sfx.stamp();
    closeSlipModal();
  });
  on("slip-cancel", "click", () => {
    ...
    if (state.gameMode === "verify")
      sendAction({ type: "verifyDecision", ticketId: tid, decision: "cancel" });
    closeSlipModal();
  });
...
function resetLocal() {
  ...
  ["board", "queue", "operators", "log", "leaderboard"].forEach((id) => {
    const el = $(id);
    if (el) el.innerHTML = "";
  });
  resetRenderCaches();
...
function renderLobby() {
  ...
  if ($("lobby-start-btn"))
    ($("lobby-start-btn") as HTMLElement).style.display = Net.isHost ? "" : "none";
  ...
  if (desc) {
    ...
    desc.textContent =
      lobbyMode === "supervisor" && state.players.length < 2
...
function updateHud() {
  ...
  if (levelNumEl) levelNumEl.textContent = String(state.levelIdx + 1);
  const correctValEl = $("correct-val");
  if (correctValEl) correctValEl.textContent = `${state.correctCount} / ${state.goal}`;
  const teamScoreEl = $("team-score");
  if (teamScoreEl) teamScoreEl.textContent = String(state.teamScore + state.teamChaos);
  const teamPenEl = $("team-pen");
  if (teamPenEl) teamPenEl.textContent = "-" + state.teamPenalty;
  const totalMax = state.players.reduce((s, p) => s + p.maxCables, 0);
  const free = state.players.reduce((s, p) => s + p.cables, 0);
  const cablesFreeEl = $("cables-free");
  if (cablesFreeEl) cablesFreeEl.textContent = `${free} / ${totalMax}`;
  const hudRoomEl = $("hud-room");
  if (hudRoomEl) hudRoomEl.textContent = Net.roomCode || "---";
  const hudModeEl = $("hud-mode");
  if (hudModeEl) {
    const m = state.gameMode || "classic";
    const iAmSv = m === "supervisor" && Net.myId === state.supervisorId;
    hudModeEl.textContent =
      m === "classic"
        ? "CLASSIC"
        : m === "verify"
          ? "VERIFY"
...
function renderBoard() {
  ...
  lvl.chars.forEach((id) => {
    ...
    if (el.classList.contains("claimed-self") !== wantSelf)
      el.classList.toggle("claimed-self", wantSelf);
    if (el.classList.contains("claimed-other") !== wantOther)
      el.classList.toggle("claimed-other", wantOther);

    let color = null;
    if (claimer) color = claimer.color;
    else if (liveTicket)
      color = state.players.find((p) => p.id === liveTicket.connection?.byPlayer)?.color;
    const prev = el.style.getPropertyValue("--claimColor");
    if (color) {
      if (prev !== color) el.style.setProperty("--claimColor", color);
...
function renderQueue() {
  ...
  ringing.forEach((t) => {
    ...
    const from = CHARS[t.from],
      to = t.to ? CHARS[t.to] : null;
    const slip = t.slip;
    const showSlipPreview =
      state.gameMode === "supervisor" || (state.gameMode === "verify" && t.approval !== "approved");
    ...
    if (!div) {
      ...
      div.addEventListener("click", () => {
        if (state.phase !== "playing") return;
        if (
          state.gameMode === "supervisor" &&
          Net.myId === state.supervisorId &&
      ...
      if (liveHead) el.insertBefore(div, liveHead);
    ...
    const wantReview =
      state.gameMode === "verify" && t.approval === "pending" && t.reviewer === Net.myId;
  ...
  live.forEach((t) => {
    ...
    if (div.style.getPropertyValue("--claimColor") !== color)
      div.style.setProperty("--claimColor", color);
  });
  ...
  state.log.forEach((entry) => {
    ...
    if (entry.status === "ended") {
      if (entry.result === "cut") {
        cls = "cut";
        txt = "EARLY CUT";
      } else if (entry.result === "routed") {
        cls = "ok";
        txt = "ROUTED";
      } else if (entry.result === "chaos") {
        cls = "chaos";
        txt = "CROSSED";
      }
    }
    ["streaming", "cut", "ok", "chaos"].forEach((c) => {
      if (c !== cls && header.classList.contains(c)) header.classList.remove(c);
    });
    if (!header.classList.contains(cls)) header.classList.add(cls);
    if (badge.textContent !== txt) badge.textContent = txt;
    const isStreaming = cls === "streaming";
    if (div.classList.contains("streaming") !== isStreaming)
      div.classList.toggle("streaming", isStreaming);
...
function renderSummary() {
  ...
  document
    ...
    .forEach((el) => ((el as HTMLElement).style.display = Net.isHost ? "" : "none"));
  const endWaitEl = $("end-wait");
  if (endWaitEl) endWaitEl.style.display = Net.isHost ? "none" : "";
  const nextBtnEl = $("next-btn");
  if (nextBtnEl)
    nextBtnEl.textContent = passed
      ? lastLevel
        ? "PLAY AGAIN ▸"
        : "NEXT SHIFT ▸"
      : "RETRY SHIFT ▸";
...
function renderCables() {
  ...
  live.forEach((t) => {
    ...
    const conn = t.connection;
    if (!conn) return;
    const a = plugRect(t.from),
      b = plugRect(conn.actualTo);
    if (!a || !b) return;
    const owner = state.players.find((p) => p.id === conn.byPlayer);
    const color = owner?.color || "#68c6ff";
    ...
    if (!g) {
      ...
      hit.setAttribute("stroke-opacity", "0.3");
      ...
      main.setAttribute("pointer-events", "stroke");
      high.setAttribute("stroke", "#fff8");
      high.setAttribute("stroke-width", "1");
      high.setAttribute("fill", "none");
      high.setAttribute("stroke-linecap", "round");
      high.setAttribute("pointer-events", "none");
      ca.setAttribute("r", "7");
      cb.setAttribute("r", "7");
    ...
    ca.setAttribute("cx", a.x);
    ca.setAttribute("cy", a.y);
    ca.setAttribute("fill", color);
    cb.setAttribute("cx", b.x);
    cb.setAttribute("cy", b.y);
...
function openSlipModal(ticket: Ticket, role: string) {
  ...
  const slipNum = $("slip-num");
  if (slipNum) slipNum.textContent = "#" + ticket.slip.slipNum;
  const slipCaller = $("slip-caller");
  if (slipCaller) slipCaller.textContent = ticket.slip.callerName;
  const slipLine = $("slip-line");
  if (slipLine) slipLine.textContent = ticket.slip.line;
  const slipReq = $("slip-req");
  if (slipReq) slipReq.textContent = ticket.slip.requestName;
  const slipFlag = $("slip-flag");
  if (slipFlag) slipFlag.style.display = ticket.slip.flagged ? "" : "none";
  const slipHint = $("slip-hint");
  if (slipHint)
    slipHint.textContent =
      role === "supervisor"
...
function openAgencyModal(ticket: Ticket) {
  ...
  const agencyQ = $("agency-q");
  if (agencyQ) agencyQ.textContent = ticket.agencyQ.text;
...
function reconcileSlipModal() {
  ...
  if (state.phase === "playing" && state.gameMode === "verify" && slipModalTicketId == null) {
    const mine = state.tickets.find(
      (t) => t.status === "ringing" && t.approval === "pending" && t.reviewer === Net.myId
    );
  ...
  if (state.gameMode === "verify") {
    if (t.approval !== "pending" || t.reviewer !== Net.myId) {
      closeSlipModal();
      return;
    }
  }
  if (state.gameMode === "supervisor") {
    if (t.approval !== "awaiting-stamp" || Net.myId !== state.supervisorId) {
      closeSlipModal();
      return;
    }
  }
