...
export interface Level {
  ...
  bin: Partial<Record<string, number>>;
  tickLimit: number;
  win: string;
}
...
const SRC = (
  ...
  offset = 1,
  loop = false
): Source => ({ kind: "source", x, y, dir, seq, period, offset, loop });
...
export const LEVELS: Level[] = [
...

  {
    ...
    brief:
      "HR put a 'wellness obstacle' in your wire path. Use corner pipes (click WIRE again to rotate, or right-click a placed pipe to cycle shape).",
    fixed: [SRC(1, 1, 1 /*E*/, [3, 3, 3], 3, 1), SNK(8, 8, 3 /*W*/, [3, 3, 3])],
    walls: [
      { x: 4, y: 4 },
      { x: 5, y: 4 },
      { x: 4, y: 5 },
...

  {
    ...
    brief:
      "Source emits weak signal (value 1). Receiver expects a robust 5. Place an AMP gate, click it, configure it to add +4. (Click placed gates to program them.)",
    fixed: [SRC(1, 5, 1, [1, 1, 1], 4, 1), SNK(8, 5, 3, [5, 5, 5])],
    walls: [],
    bin: { pipe: 10, amp: 1 },
    tickLimit: 50,
...

  {
    ...
    fixed: [SRC(1, 1, 2, [1, 2, 3], 3, 1), SNK(8, 8, 0, [2, 4, 6])],
    walls: [
      { x: 4, y: 4 },
      { x: 5, y: 5 },
    ],
...

  {
    ...
    brief:
      "The receiver is a snob. It only wants even values. The source is firing 1..6. Use a FILTER to drop odd ones.",
    fixed: [SRC(1, 5, 1, [1, 2, 3, 4, 5, 6], 3, 1), SNK(8, 5, 3, [2, 4, 6])],
    walls: [],
    bin: { pipe: 10, filt: 1 },
    tickLimit: 80,
...

  {
    ...
    brief:
      "Two receivers, both need the same signal. Use a T-SPLIT pipe to fork the wire. (Right-click a wire to cycle through pipe shapes.)",
    fixed: [SRC(0, 5, 1, [4, 4, 4], 3, 1), SNK(9, 1, 3, [4, 4, 4]), SNK(9, 8, 3, [4, 4, 4])],
    walls: [],
    bin: { pipe: 18 },
    tickLimit: 80,
...

  {
    ...
    brief:
      "Two sources, one receiver. The receiver expects 3, 7. But signals arrive at the wrong times. Use a DELAY gate so signals arrive in the right order.",
    fixed: [
...

  {
    ...
    brief:
      "Source emits 1..6. Even values must reach the BOTTOM receiver. Odd values must reach the TOP receiver. Use a ROUTER (configure: split on '%2').",
    fixed: [
...

  {
    ...
    brief:
      "Source emits 1, 2, 3. Two receivers need different versions. TOP wants DOUBLED (2,4,6). BOTTOM wants MINUS ONE (0,1,2). Fork the signal through a T-junction and transform each branch independently.",
    fixed: [SRC(0, 4, 1, [1, 2, 3], 3, 1), SNK(9, 1, 3, [2, 4, 6]), SNK(9, 7, 3, [0, 1, 2])],
    walls: [],
...

  {
    ...
    brief:
      "Source emits 1..6. Router splits by parity. ODDS (1,3,5) go to TOP as-is. EVENS (2,4,6) get DOUBLED (4,8,12) and go to BOTTOM. It's your last day. Make us cry.",
    fixed: [
      SRC(0, 4, 1, [1, 2, 3, 4, 5, 6], 3, 1),
      SNK(9, 1, 3, [1, 3, 5]),
      SNK(9, 7, 3, [4, 8, 12]),
