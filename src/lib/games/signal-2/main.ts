...
function makeTile(type: string, props: Partial<Tile> = {}): Tile {
  ...
  if (type === 'filt' && props.op == null) { t.op = '%2=='; t.n = 0; }
  if (type === 'delay' && props.n == null) t.n = 1;
  if (type === 'router' && props.op == null) { t.op = '%2=='; t.n = 0; }
  return t;
}
...
function resetSinks() {
  ...
  state.level.fixed.filter((f): f is Sink => f.kind === 'sink').forEach(snk => {
    const k = snk.x + ',' + snk.y;
    state.results[k] = [];
  });
}
...
function renderGoals() {
  ...
  ul.innerHTML = '';
  if (!state.level) return;
  state.level.fixed.filter((f): f is Sink => f.kind === 'sink').forEach((snk, i) => {
    const k = snk.x + ',' + snk.y;
...
function resetSimState() {
  ...
  if (!state.level) return;
  state.level.fixed.filter((f): f is Source => f.kind === 'source').forEach(s => {
...
function handleEnter(tile: Tile, sig: Signal, nx: number, ny: number, travelDir: number, outArr: Signal[]) {
  // Sink: absorb
  if (tile.type === 'sink') {
    const k = nx + ',' + ny;
  ...
  if (tile.type === 'router') {
    ...
    const exit0 = exits[0];
    log(`⟚ROUTER (${nx},${ny}) ${val} ${tile.op}${tile.n}=${cond?'T':'F'} → ${DIR_NAMES[exit0 ?? 0] ?? '?'}`, 'info');
  }
}

function advance(tile: Tile, val: number, nx: number, ny: number, travelDir: number, outArr: Signal[], sig: Signal) {
  const exits = tileExits(tile, travelDir, val);
  if (exits.length === 0) {
