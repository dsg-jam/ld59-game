...
function setLobbyStatus(t: string) {
  $("lobby-status").textContent = t;
}
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "ArrowLeft") {
    ($("offset") as HTMLInputElement).value = String(
      Math.max(OFFSET_MIN, Number(($("offset") as HTMLInputElement).value) - ARROW_KEY_STEP)
    );
    $("offset").dispatchEvent(new Event("input"));
  }
  if (e.code === "ArrowRight") {
    ($("offset") as HTMLInputElement).value = String(
      Math.min(OFFSET_MAX, Number(($("offset") as HTMLInputElement).value) + ARROW_KEY_STEP)
    );
    $("offset").dispatchEvent(new Event("input"));
  }
});
