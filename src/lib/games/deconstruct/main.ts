...
type MoveResult = { slot: number; card: Card | null; sel: [number, number][] | null; points: number };
...
type BlockRiseAnim = {
    type: 'blockRise'; mesh: THREE.Mesh; edge: THREE.LineSegments;
    clonedMat: THREE.MeshStandardMaterial; vel: number; rot: number; opacity: number; t: number;
};
type SignalRingAnim = {
    type: 'signalRing'; mesh: THREE.Mesh; mat: THREE.MeshBasicMaterial;
    geo: THREE.RingGeometry; t: number; duration: number;
};
...
function setSeed(s: number) { _rng = mulberry32(s); }
const rand = () => _rng();

// ---- Constants ----
const W = 6, H = 6, D = 4;
const COLORS_HEX: Record<ColKey, number> = { R: 0xee5555, G: 0x55cc55, B: 0x5599ff, Y: 0xeedd55, P: 0xcc66ff };
const COLORS_CSS: Record<ColKey, string> = { R: '#ee5555', G: '#55cc55', B: '#5599ff', Y: '#eedd55', P: '#cc66ff' };
const COLOR_NAMES: Record<ColKey, string> = { R: 'ALPHA', G: 'BETA', B: 'GAMMA', Y: 'DELTA', P: 'EPSILON' };
...
const HS: {
    seed: number; grid: ColKey[][][]; hands: Record<number, Card[]>;
    picks: Record<number, Pick>; scores: Record<number, number>;
    turn: number; playerIds: string[]; playerNames: Record<number, string>;
    started: boolean; N: number;
} = {
    seed: 0,
...
// Local display state
const S: {
    grid: ColKey[][][]; myHand: Card[]; selectedCardIdx: number | null;
    selected: [number, number][]; allScores: ScoreEntry[];
    turn: number; locked: boolean; playerCount: number; playerNames: Record<number, string>;
} = {
...
function buildGrid() {
    while (constructGroup.children.length > 0) {
        ...
        if (child) constructGroup.remove(child); else break;
    }
    meshGrid = [];
    antennaTips = [];
    for (let x = 0; x < W; x++) {
...
function buildSeats(n: number) {
    while (seatGroup.children.length > 0) {
        ...
        if (child) seatGroup.remove(child); else break;
    }
    seatMeshes = [];
    for (let i = 0; i < n; i++) {
...
function validates(sel: [number, number][], card: Card): boolean {
    ...
    const norm = function (a: [number, number][]) {
        const mx = Math.min.apply(null, a.map(function (p) { return p[0] ?? 0; }));
        const my = Math.min.apply(null, a.map(function (p) { return p[1] ?? 0; }));
        return a.map(function (p) { return [(p[0] ?? 0) - mx, (p[1] ?? 0) - my] as [number, number]; }).sort(function (a, b) { return (a[0] ?? 0) - (b[0] ?? 0) || (a[1] ?? 0) - (b[1] ?? 0); });
    };
    ...
    for (let r = 0; r < 4; r++) {
        ...
        cells = cells.map(function (c) { return [-(c[1] ?? 0), c[0] ?? 0] as [number, number]; });
    }
    return false;
}
...
function cpuFind(card: Card): [number, number][] | null {
    ...
    for (let r = 0; r < 4; r++) {
        for (const anchor of tops) {
            const mx = Math.min.apply(null, cells.map(function (p) { return p[0] ?? 0; }));
            const my = Math.min.apply(null, cells.map(function (p) { return p[1] ?? 0; }));
            const placed: [number, number][] = cells.map(function (c) { return [(c[0] ?? 0) - mx + (anchor[0] ?? 0), (c[1] ?? 0) - my + (anchor[1] ?? 0)]; });
            if (placed.every(function (p) { return (p[0] ?? 0) >= 0 && (p[0] ?? 0) < W && (p[1] ?? 0) >= 0 && (p[1] ?? 0) < H && topColor(p[0] ?? 0, p[1] ?? 0) === card.color; })) return placed;
        }
        cells = cells.map(function (c) { return [-(c[1] ?? 0), c[0] ?? 0]; });
    }
    return null;
}
...
function hostStartRound() {
    ...
    for (let i = 0; i < n; i++) {
        const msg: RoundStartMsg = { type: 'round-start', grid: HS.grid, hand: HS.hands[i] ?? [], turn: HS.turn, scores, playerCount: n, yourSlot: i, names: HS.playerNames };
...
function hostResolveTurn() {
    const n = HS.N;
    const actions: Array<{ slot: number; card: Card | null; sel: [number,number][] | null; init: number }> = [];
    for (let i = 0; i < n; i++) {
    ...
    if (gameOver) {
        let best = -1;
        for (let i = 0; i < n; i++) {
            if ((HS.scores[i] ?? 0) > best) { best = HS.scores[i] ?? 0; winnerSlot = i; }
        }
        let tiedCount = 0;
...
type RoundStartMsg = { type: string; grid: ColKey[][][]; hand: Card[]; turn: number; scores: ScoreEntry[]; playerCount: number; names: Record<number,string>; yourSlot: number };
type TurnResultMsg = { results: MoveResult[]; grid: ColKey[][][]; scores: ScoreEntry[]; turn: number; gameOver: boolean; winnerSlot: number; yourSlot: number; names: Record<number,string> };
...
function renderUI() {
    ...
    (S.myHand || []).forEach(function (card, idx) {
        ...
        const mx = Math.max.apply(null, card.shape.cells.map(function (p) { return p[0] ?? 0; })) + 1;
        const my = Math.max.apply(null, card.shape.cells.map(function (p) { return p[1] ?? 0; })) + 1;
...
function updatePlayerList() {
    ...
    if (lobbyStatus) lobbyStatus.textContent = HS.playerIds.length + '/' + MAX_PLAYERS + ' operators tuned in.';
...
type DeconWin = Window & typeof globalThis & {
    soloGame: () => void; hostGame: () => void; hostStartNow: () => void;
    joinGame: () => void; onPickCard: () => void; onClear: () => void; onPass: () => void;
};
...
dw.hostGame = function () {
    ...
    peer.on('open', function () {
        const lobbyStatus = document.getElementById('lobby-status');
        if (lobbyStatus) lobbyStatus.textContent = 'Broadcasting — awaiting operators (' + HS.playerIds.length + '/' + MAX_PLAYERS + ')';
    });
    peer.on('connection', function (c) {
        ...
        c.on('data', function (data: unknown) {
            const msg = data as GameMsg;
            if (msg.type === 'name') {
                HS.playerNames[slot] = ((msg.name as string | undefined) ?? '').slice(0, 16) || 'P' + (slot + 1);
                updatePlayerList();
            } else {
                onMessage(data, slot);
            }
...
function animate() {
    ...
    seatMeshes.forEach(function (s, i) {
        if (s.disc) (s.disc.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.2 + Math.sin(Date.now() * 0.003 + i) * 0.1;
    });

    // Antenna tip pulse
    const tNow = Date.now() * 0.004;
    antennaTips.forEach(function (a) {
        (a.mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.55 + Math.sin(tNow + a.phase) * 0.45;
    });
    ...
    if (S.selected && S.selected.length) {
        ...
        for (const pos of S.selected) {
            ...
            if (entry) (entry.mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.4 + 0.3 * Math.sin(pulseT);
        }
    }
